...
def generate_astar(vision_radius):
    output_file = Path(__file__).parent.parent / "src" / "sensei" / f"Dijkstra{vision_radius}.java"
...
public class Dijkstra{vision_radius} {{
    ...
    private static double remainingDistance{id};
    private static double totalScore{id};
        """.rstrip()
    ...
    for (dx, dy) in offsets_by_distance:
        ...
        remainingDistance{id} = location{id}.distanceSquaredTo(target);
        totalScore{id} = distance{id} + remainingDistance{id};
        """.rstrip()
    ...
    for (dx, dy) in offsets_by_distance:
        ...
        if (rc.canMove({direction(dx, dy)})) {{
            distance{id} = myLocation.distanceSquaredTo(location{id});
        ...
        for i in range(len(predecessors)):
            ...
            content += f"""
{indent}int weight{i} = location{predecessor}.distanceSquaredTo(location{id});
{indent}if (distance{predecessor} + weight{i} < distance{id}) {{
{indent}    distance{id} = distance{predecessor} + weight{i};
{indent}    direction{id} = direction{predecessor};
{indent}    totalScore{id} = distance{id} + remainingDistance{id};
{indent}}}
            """.rstrip()
    ...
    for _ in range(len(score_vars) - 1):
        score_vars = [f"Math.min({score_vars[0]}, {score_vars[1]})"] + score_vars[2:]
...
def main():
    for vision_radius in [6, 13, 20]:
        generate_astar(vision_radius)
